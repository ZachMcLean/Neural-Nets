...
input_dim = 1
num_classes = 2
hidden_size = 64   # Adjusted so that parameter count matches LSTM Many-to-One
batch_size = 40
num_epochs = 300
learning_rate = 0.001
...
# Define a Many-to-Many LSTM model
...
model = LSTM_Many2Many(input_dim, hidden_size, num_classes)
print("LSTM Many-to-Many Model:")
print(model)
...
for epoch in range(num_epochs):
    ...
    for inputs, labels in train_loader:
        optimizer.zero_grad()
        outputs = model(inputs)  # (batch, seq_length, num_classes)
...
torch.save(history, "logs/history_lstm_m2m.pt")
print("Training complete. History saved to logs/history_lstm_m2m.pt")
